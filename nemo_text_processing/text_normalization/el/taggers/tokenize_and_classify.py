...
class ClassifyFst(GraphFst):
    """
    Final class that composes all other classification grammars for Greek.
    ...
    def __init__(
        ...
        if cache_dir is not None and cache_dir != "None":
            ...
            far_file = os.path.join(
                cache_dir,
                f"_{input_case}_el_tn_{deterministic}_deterministic{whitelist_file}.far",
            )
        if not overwrite_cache and far_file and os.path.exists(far_file):
            self.fst = pynini.Far(far_file, mode="r")["tokenize_and_classify"]
            logger.info(f"ClassifyFst.fst was restored from {far_file}.")
        else:
            logger.info(f"Creating ClassifyFst grammars for Greek. This might take some time...")
            ...
            # Combine all classifiers with weights
